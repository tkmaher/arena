...
import { isBlock, isChannel, isGroup } from "@/scripts/utility";

type AnyNode   = Block | Channel | User | Group;
type AnyStatus = ConnectionStatus | ChildrenStatus | FollowersStatus | FollowingStatus;

interface NodeListProps {
  list: AnyNode[]; status: AnyStatus;
  checkNodeVisible: (id: string) => boolean;
  onToggle: (node: AnyNode) => void; onSelect: (node: AnyNode) => void;
  loadMore: () => void; label: string; nodeId: string; limitSize?: (val: boolean) => void;
}

const iconFor  = (n: AnyNode) => isBlock(n) ? "block.svg" : isChannel(n) ? "channel.svg" : isGroup(n) ? "group.svg" : "user.svg";
...
export default function NodeList({ list, status, checkNodeVisible, onToggle, onSelect, loadMore, label, nodeId, limitSize }: NodeListProps) {
  const [open,     setOpen]     = useState(!limitSize);
  ...
  const filtered   = list.filter(n => (n.title ?? n.id.toString()).toLowerCase().includes(search.toLowerCase()));

  useEffect(() => { setFetching(false); }, [status, list]);
  useEffect(() => { limitSize?.(open); }, [open, limitSize]);

  return (
    <motion.div className="collection-col"
      initial={didAnimate.has(animKey) ? false : { opacity: 0, y: -8 }}
      animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
      onAnimationComplete={() => didAnimate.add(animKey)}
    >
      <div className="collection-header" data-open={String(open)} onClick={() => setOpen(o => !o)}>
      ...
      <div className={`collection-body${open ? "" : " closed"}`}>
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          placeholder={`Search ${label.toLowerCase()}...`} className="text-input" />

        <div className="collection-list">
          {filtered.length === 0 ? <em>[No {label.toLowerCase()} found]</em> : filtered.map(node => (
            <div key={node.id} className="checklist"
              onMouseEnter={() => setHovered(node.id)} onMouseLeave={() => setHovered(null)}>
              <a onClick={() => onSelect(node)}>
                <img src={iconFor(node)} />
                <div>{node.title ?? node.id}</div>
              </a>
              {!limitSize && <input type="checkbox" checked={checkNodeVisible(node.id)} onChange={() => onToggle(node)} />}
              <div className="tooltip info-sub" style={{
                opacity:    node.id === hovered ? 1 : 0,
                visibility: node.id === hovered ? "visible" : "hidden",
                right:      node.id === hovered ? "0.8em" : "5em",
              }}>
                {hasImage(node) && <div className="tooltip-img" style={{ backgroundImage: `url(${(node as ImageBlock).thumbnailUrl})` }} />}
        ...
        {!isComplete && (
          <button className={`loader${fetching ? " disabled" : ""}`}
